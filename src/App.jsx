...
function App() {
    ...
    return <>
        ...
        <section className="dashboard" aria-busy={status.type === "loading"}>
            ...
            {outdoor && <><article className="outdoor-card"><div><p className="eyebrow">{t("outdoorWindow")}</p><h2>{outdoor.good ? t("outdoorGood") : t("outdoorAvoid")}</h2></div><strong>{outdoor.time}</strong></article><TemperatureChart forecast={forecast} temp={temp} title={t("temperatureChart")} /></>}
        ...
        {favoritesOpen && <aside className="favorites-panel"><div className="section-heading"><h2>{t("favorites")}</h2><button className="close-button" onClick={() => setFavoritesOpen(false)}>×</button></div>{favorites.length ? favorites.map(item => <button className="favorite-item" key={item} onClick={() => { setInput(item); loadCity(item); }}>{item}</button>) : <p className="empty-favorites">{t("noFavorites")}</p>}</aside>}
        </main>
    </>;
}
